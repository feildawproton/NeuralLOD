...
def define_model(width = 64, drop_ratio = 0.1):
	...
	return tf.keras.Model(inputs = input, outputs = [output_3])

#adjustments made for multiple outputs
image		= image.imread(os.path.join("shapes", "bravenewworld.png"))
image		= np.expand_dims(image, axis = -1)
print(image.shape)
img_repeat	= np.repeat(image, 1, axis = -1)
